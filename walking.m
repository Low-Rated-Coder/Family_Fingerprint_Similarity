...
% @Inputs:
%   im: image data, h by w
%   mask: 0, background; 1, foreground
%   orientim: pixel-wise orientation field of im, share the same size with im.
%   step: length of one walking step, default to 7 pixel
%   n: number of starting points sampled along x-axis or y, default to 2
%   Td: threshold for stopping the walking process, default to 2 pixel
%   R: radius of the neighborhood of the candidate singular point,default to 16 pixel
% @Outputs:
%   sps: a structure with fields 'core' and 'delta', containing the detected singular points
%   time: processing time of the walking algorithm excluding the segmentation and orientation field estimation.
...
%   im: image data, h by w
%   mask: 0, background; 1, foreground
%   dfim: pixel wise directional field of im. Not neccisarily be in [0, pi).
%   start: starting point
%   step: length of one walking step, default to 7 pixel
%   Td: threshold for stopping the walking process, default to 2 pixel
% @Return:
%   sp: singular point, if failed , sp = [];
%   path: walking path, a k-by-2 matrix, each row corresponds to a point in the walking path.
...
%   im: image data, h by w
%   mask: 0, background; 1, foreground
%   orientim: pixel wise directional field of im. Not neccisarily be in [0, pi).
%   tempsp: the candidate singular point to be checked.
%   step: length of one walking step, default to 7 pixel
%   Td: threshold for stopping the walking process, default to 2 pixel
%   R: radius of the neighborhood of the candidate singular point tempsp, default to 16 pixel
% @Return:
%   stable: 1, tempsp is genuine; 0, spurious
...
% Inputs:
%   points: points to be merged, a n by 2 matrix.
%   threshold: the threshold for mergement.
% Outputs:
%   points: merged points, a m by 2 matrix, 1<=m<=n.
